...
"""
    This is a program that uses a function with the header;
                def shuffle(matrix)
    to shuffle the rows in the two dimwnsional list given below;
...
def main():
    ...
    [[print(f"{matrix[row][column]} ",end="") for column in range(len(matrix[row]))] and print() or row for row in range(len(matrix))]
